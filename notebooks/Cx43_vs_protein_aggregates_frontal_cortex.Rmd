---
title: "Cx43 vs protein aggregates: frontal cortex"
output:
  html_document:
    df_print: paged
editor_options:
...
# Load data
```{r}
dat <- read.csv("~/Cam_Dropbox/Nat_Hastings/PD_data/Cx43_vs_aggregates_frontal_cortex.csv", header=T, na.string="N/A")
...
```{r}
tbl <- as.data.frame(table(dat$Am.b))
names(tbl) <- c("Amyloid beta", "Frequency")
knitr::kable(tbl, caption="Amyloid beta", format = "html", table.attr = "style='width:30%;'")
```
...
```{r}
tbl <- as.data.frame(table(dat$Am.b[group=="P"]))
names(tbl) <- c("Amyloid beta", "Frequency")
knitr::kable(tbl, caption="Amyloid beta", format = "html", table.attr = "style='width:30%;'")
```
...
```{r}
tbl <- as.data.frame(table(dat$Am.b[group=="C"]))
names(tbl) <- c("Amyloid beta", "Frequency")
knitr::kable(tbl, caption="Amyloid beta", format = "html", table.attr = "style='width:30%;'")
```
...
### Correlation analysis
```{r}
corMat <- rcorr(as.matrix(dat[group=="C",c(1,3,4)]), type="spearman")
corrplot(corMat$r, method="circle")
corMatTable <- round(corMat$r, 3)
corMatTable[upper.tri(corMatTable, diag=TRUE)] <- ""
corMatP <- round(corMat$P, 3)
